 ...
 * ****************************************************************
 */
public class MaterPagedListAdapter extends PagedListAdapter<MaterInfoEntity, RecyclerView.ViewHolder> {

    private ClickCallback clickCallback;
    private NetworkState networkState = null;

    public MaterPagedListAdapter(@NonNull ClickCallback clickCallback) {
        super(new DiffUtil.ItemCallback<MaterInfoEntity>() {
            //这个是用来判断是否是一个对象的
            @Override
            public boolean areItemsTheSame(@NonNull MaterInfoEntity oldItem, @NonNull MaterInfoEntity newItem) {
                return oldItem.getMater_id() == (newItem.getMater_id());
            }

            //这个是用来判断相同对象的内容是否相同 和UI展示的相同
            @Override
            public boolean areContentsTheSame(@NonNull MaterInfoEntity oldItem, @NonNull MaterInfoEntity newItem) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == R.layout.item_network_state) {
            ...
            return new MyViewHolder(DataBindingUtil.inflate(LayoutInflater.from(parent.getContext()),
                    R.layout.item_mater_paging, parent, false));
        }
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof NetworkStateItemViewHolder) {
            ...
            MaterInfoEntity data = getItem(position);
...
//            }
    ...
    public int getItemViewType(int position) {
        if (hasExtraRow() && position == getItemCount() - 1) {
            ...
            return R.layout.item_mater_paging;
        }
    }
    ...
    public static class MyViewHolder extends RecyclerView.ViewHolder {
        private ItemMaterPagingBinding binding;

        private MyViewHolder(ItemMaterPagingBinding binding) {
            super(binding.getRoot());
            this.binding = binding;
        }
    }
    ...
    public interface ClickCallback {
        void onClick(@NonNull MaterInfoEntity data);

        void onLongClick(@NonNull MaterInfoEntity data);
